...
// JANK ytcrossfade.BUFFER_TIME = 8; // Time to buffer, in seconds.
ytcrossfade.BUFFER_TIME = 4; // Time to buffer, in seconds.
...
ytcrossfade.loadNext = function(ytid) {
  ...
  nextUp.stopVideo();
  // JANK ytcrossfade.upNextLoaded = true;
  ytcrossfade.upNextLoaded = ytid;
  return 1;
}
...
ytcrossfade.stopwatch = function() {
  // JANK -commented out below in hopes of using the better way to get now playings
  // if  (document.getElementsByClassName('ytcrossfade-nowPlaying')[0].id.substring(19) == 'a') {
  ...
  try {
    var remaining = ytcrossfade.getCurrentPlayer().getDuration() - ytcrossfade.getCurrentPlayer().getCurrentTime();
  } catch(err) {
    log('ERROR!');
    log(err);
    return -1;
  ...
  // JANK -- replacing all the buffering code to just play next w/o buffer.
  ...
  //   ytcrossfade.playNext();
  // }
  // if ((remaining < ytcrossfade.CROSSFADE_TIME + ytcrossfade.BUFFER_TIME)
  //     && (ytcrossfade.playlist.length > 0)) {
  //   if ( ytcrossfade.upNextLoaded != ytcrossfade.playlist[0].videoId ) {
  //     log('doing a loadNext to BUFFER BITCHES');
  //     // It isn't so load the correct one. This just prevents the buffer thing from working.
  //     var trash = ytcrossfade.loadNext();    
  //   }
  // }
  // else 
  if ((remaining > 0) && (remaining < ytcrossfade.CROSSFADE_TIME)) {
    log('time remaining: ' + remaining);
    log('AASDASDANLSKJDAKJSDNLAJKSNDLKJANDJKNASK'); 
    ytcrossfade.log('starting a load next AND a play next');
    var trash = ytcrossfade.loadNext();    
    ytcrossfade.playNext();
    heyLetsDj.skip();
  }
